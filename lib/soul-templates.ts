 ...
 *   1. `/soul-templates.json` — built from
 *      github.com/mergisi/awesome-openclaw-agents via
 *      `scripts/build-soul-templates.mjs`. Treat as upstream / regenerated.
...
export interface SoulTemplateCatalog {
  version: string;
  /**
   * Unique stamp emitted by `scripts/build-soul-templates.mjs` on every
   * rebuild. The loader uses this for cache busting; UIs can ignore it.
   */
  build_id?: string;
  curated_at: string;
  source: string;
...
// browser cache. The id changes every time `scripts/build-soul-templates.mjs`
// runs, and because the constant is baked into the JS bundle, the bundle
