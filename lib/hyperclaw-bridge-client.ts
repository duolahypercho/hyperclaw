...
export async function listProjects(status?: string): Promise<BridgeProject[]> {
  const result = await bridgeInvoke("project-list", status ? { status } : {});
...
export interface BridgeWorkflowStep {
  id: string;
  name: string;
  stepType: "manual_trigger" | "agent_task" | "human_approval" | "notification" | "sql_query" | "chart" | "component" | string;
  dependsOn?: string[];
