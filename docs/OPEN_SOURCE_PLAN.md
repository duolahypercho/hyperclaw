...
That path was not wired through `electron/main.js` or `preload.js`. The bundled
ONNX weights added roughly 70+ MB to the repo and build artifacts, so they were
removed alongside the voice UI surface for the local-first release.
