...
export function useConnectorStatus(): UseConnectorStatusResult {
  ...
  useEffect(() => {
    return subscribeGatewayConnection(() => {
      ...
      // `permanentlyFailed` lives on the singleton. Read it via a side-channel.
      const failed = (s as unknown as { permanentlyFailed?: boolean })
        .permanentlyFailed;
      if (typeof failed === "boolean") setPermanentlyFailed(failed);
    });
  }, []);
  ...
  // Periodically probe end-to-end health when hub WS is up.
  useEffect(() => {
    if (!hubConnected) {
      setProbe((p) => ({ ...p, healthy: null }));
      return;
    }
    let cancelled = false;
    ...
    const runProbe = async () => {
      ...
      const result = await probeGatewayHealth(PROBE_TIMEOUT_MS);
...
interface DeriveInput {
  ...
  probe: { healthy: boolean | null; ms: number; at: number };
}
...
function deriveStatus({
  ...
  if (devices === null) {
    return hubConnected ? { state: "connecting" } : { state: "connecting" };
  }
  ...
  // Hub WS up and device online — consult probe.
