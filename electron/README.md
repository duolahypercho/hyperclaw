...
macOS quarantines unsigned apps. If the unzipped **Hyperclaw.app** won’t open:

**Option 1 – Terminal (recommended):**
```bash
xattr -cr /path/to/Hyperclaw.app
open /path/to/Hyperclaw.app
```

**Option 2 – Helper script (from repo root):**
```bash
chmod +x electron/scripts/open-mac-app.sh
./electron/scripts/open-mac-app.sh /path/to/Hyperclaw.app
```

**Option 3 – Finder:** Right-click **Hyperclaw.app** → **Open** → confirm “Open” in the dialog (one-time).
...
- If it still exits: run from Terminal to see the error:
  ```bash
  /path/to/Hyperclaw.app/Contents/MacOS/Hyperclaw
  ```
  Or check `~/.openclaw/copanion-crash.log` for uncaught errors.
- If you had Hyperclaw open before (e.g. in the tray), only one instance is allowed; the second launch will quit immediately. Quit the first instance fully, then open again.
