...
When the user opens the Hyperclaw app:
...
3. **main.js** then loads a URL in that window. In remote mode that URL is your Vercel domain (e.g. `https://your-domain.example.com/dashboard`). So the **content** (HTML, JS, CSS) comes from Vercel, but the **window** still belongs to Electron and already has preload run.
...
┌─────────────────────────────────────────────────────────────────┐
│  You                                                             │
│  1. Deploy Next.js app to Vercel  →  https://your-domain.example.com
│  2. Build Electron app in "remote" mode  →  Hyperclaw-Setup-Remote.exe / .app
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│  User                                                            │
│  1. Installs and opens the Electron app                          │
│  2. The app window loads: https://your-domain.example.com/dashboard │
...
1. **Deploy to Vercel**  
   Connect your repo and deploy the Next.js app. Note the URL (e.g. `https://your-domain.example.com`).

2. **Point Electron at that URL**  
   In `electron/app-config.json` (or whatever sets config for the build), set:
   - `mode`: `"remote"`
   - `remoteUrl`: your Vercel URL (e.g. `"https://your-domain.example.com"`)
...
**"Origin not allowed" (Control UI)**: The Hyperclaw Electron app **automatically** adds your app origin (from `electron/app-config.json` `remoteUrl`) to `~/.openclaw/openclaw.json` → `gateway.controlUi.allowedOrigins` when:
...
So in normal use, users do not need to edit the file by hand. If you still see *origin not allowed*, then:
  ...
  1. Open `~/.openclaw/openclaw.json` and ensure `gateway.controlUi.allowedOrigins` includes your app URL (no trailing slash), e.g. `["https://your-domain.example.com"]`.
