...
const MainLayoutInner = ({ children }: any) => {
  ...
  const hasElectronLocalBootstrap = typeof window !== "undefined" &&
    !!window.electronAPI?.runtimes?.installLocalConnector;
  const canProbeLocalBridgeForGuest = typeof window !== "undefined" && (() => {
  ...
  const { isLoading, isRedirecting } = useAuthGuard({
    ...
    delay: 100,
    allowUnauthenticated: canUseGuestLocalMode || isProbingLocalGuestMode,
  });
  ...
  useEffect(() => {
    if (hasElectronLocalBootstrap) {
      setLocalBridgeProbeComplete(true);
      return;
    }
    if (!canProbeLocalBridgeForGuest) {
      setLocalBridgeProbeComplete(true);
      return;
    }

    let cancelled = false;
    fetch("http://127.0.0.1:18790/bridge/health", {
