...
export function useEditorActions(
  ...
  const handleRotateSelected = useCallback(() => {
    // If in furniture placement mode, cycle the selected type through the rotation group or ghost rotation
    if (editorState.activeTool === EditTool.FURNITURE_PLACE) {
      ...
      if (rotated) {
        ...
        editorState.ghostRotation = (editorState.ghostRotation + 90) % 360
      }
      setEditorTick((n) => n + 1)
      return
    }
  ...
  const handleEditorTileAction = useCallback((col: number, row: number) => {
    ...
    if (editorState.activeTool === EditTool.TILE_PAINT) {
      ...
      if (type === '') {
        ...
        if (editorState.ghostRotation !== 0) {
          placed.rotation = editorState.ghostRotation as 0 | 90 | 180 | 270
        }
