...
import { shouldPreferBrowserScreenshot } from "../externalTypes";
...
function useBrowserPreviewScreenshot(params: {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/office/browser-preview?${queryParams.toString()}`, {
        cache: "no-store",
      });
      ...
      setSnapshot((current) => ({
        ...
        browserUrl,
        error:
          caughtError instanceof Error ? caughtError.message : "Unable to capture browser preview.",
        loading: false,
      }));
    }
  }, [browserUrl, enabled]);
