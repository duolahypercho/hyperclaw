...
import type { AgentAvatarProfile, JanitorActor, RenderAgent } from "../core/types";
import { AgentModelProps } from "./types";

/** Inline stub - generates a deterministic default avatar profile from an agent ID. */
function createDefaultAgentAvatarProfile(agentId: string): AgentAvatarProfile {
  let hash = 0;
  for (let i = 0; i < agentId.length; i++) {
    hash = (hash * 31 + agentId.charCodeAt(i)) | 0;
  }
  const skinTones = ["#f5d0a9", "#d4a574", "#8d5524", "#c68642", "#e0ac69"];
  const hairColors = ["#2c1b0e", "#6b4423", "#d4a574", "#1a1a2e", "#8b0000"];
  const topColors = ["#3b82f6", "#ef4444", "#22c55e", "#f59e0b", "#8b5cf6", "#ec4899"];
  const bottomColors = ["#1e3a5f", "#374151", "#1f2937", "#4a3728"];
  const hairStyles: AgentAvatarProfile["hair"]["style"][] = ["short", "parted", "spiky", "bun"];
  const topStyles: AgentAvatarProfile["clothing"]["topStyle"][] = ["tshirt", "hoodie", "jacket"];
  const bottomStyleOptions: AgentAvatarProfile["clothing"]["bottomStyle"][] = ["pants", "shorts", "cuffed"];
  const abs = Math.abs;
  return {
    body: { skinTone: skinTones[abs(hash) % skinTones.length] },
    hair: {
      style: hairStyles[abs(hash >> 3) % hairStyles.length],
      color: hairColors[abs(hash >> 5) % hairColors.length],
    },
    clothing: {
      topStyle: topStyles[abs(hash >> 7) % topStyles.length],
      topColor: topColors[abs(hash >> 9) % topColors.length],
      bottomStyle: bottomStyleOptions[abs(hash >> 11) % bottomStyleOptions.length],
      bottomColor: bottomColors[abs(hash >> 13) % bottomColors.length],
      shoesColor: "#1a1a2e",
    },
    accessories: {
      glasses: abs(hash >> 15) % 4 === 0,
      headset: abs(hash >> 17) % 5 === 0,
      backpack: abs(hash >> 19) % 6 === 0,
      hatStyle: "none",
    },
  };
}
...
export const AgentModel = memo(function AgentModel({
  ...
  // --- Avatar image texture (loaded from avatarUrl) ---
  const [avatarTexture, setAvatarTexture] = useState<THREE.Texture | null>(null);
  const avatarUrlRef = useRef(avatarUrl);
  const avatarTextureRef = useRef<THREE.Texture | null>(null);
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      undefined,
      () => setAvatarTexture(null), // failed to load — fall through to emoji/none
    );
    return () => setAvatarTexture(null);
  }, [avatarUrl]);

  useEffect(() => {
    const prevTexture = avatarTextureRef.current;
    if (prevTexture && prevTexture !== avatarTexture) {
      prevTexture.dispose();
    }
    avatarTextureRef.current = avatarTexture;
    return () => {
  ...
  return (
    ...
    >
      ...
      {/* Floating avatar billboard above the agent */}
      {!activeSpeechBubble && (avatarTexture || avatarEmoji) ? (
        <Billboard position={[0, avatarTexture ? 1.42 : 1.38, 0]}>
          {avatarTexture ? (
            <>
            ...
            <>
              {/* Dark circular backdrop for emoji */}
              <mesh position={[0, 0, -0.001]}>
      ...
      {!activeSpeechBubble && name ? (
        <Billboard position={[0, 1.05, 0]}>
          ...
          <Text
            ...
            whiteSpace="nowrap"
          >
            {(() => { const n = name.replace(/\s+/g, " ").trim(); return n.length > 9 ? n.slice(0, 8) + "…" : n; })()}
          </Text>
        </Billboard>
      ) : null}
