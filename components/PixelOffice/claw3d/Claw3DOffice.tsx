...
import { AtmImmersiveScreen } from "./screens/AtmImmersiveScreen";
import { GithubImmersiveScreen } from "./screens/GithubImmersiveScreen";
import {
  PhoneBoothImmersiveScreen,
  type PhoneCallStep,
} from "./screens/PhoneBoothImmersiveScreen";
import {
  SmsBoothImmersiveScreen,
  type TextMessageStep,
} from "./screens/SmsBoothImmersiveScreen";
import { StandupImmersiveScreen } from "./screens/StandupImmersiveScreen";
import {
  buildMockPhoneCallScenario,
  buildMockTextMessageScenario,
  extractSpeechImage,
  type MockPhoneCallScenario,
  type MockTextMessageScenario,
  type OfficeDeskMonitor,
  type OfficeAnimationState,
  type StandupMeeting,
  type SkillStatusEntry,
  type OfficeCleaningCue,
  type OfficeLayoutSnapshot,
  type OfficeUsageAnalyticsParams,
} from "./externalTypes";
import { MonitorImmersiveContent as MonitorImmersiveOverlay } from "./overlays/MonitorImmersiveContent";
import {
  AGENT_RADIUS,
  BUMP_FREEZE_MS,
  BUMP_RECOVERY_MS,
...
import type { NavGrid } from "./core/navigation";
import { AgentModel as AgentObjectModel } from "./objects/agents";
import {
...
function AdaptiveDprController() {
  ...
  useEffect(() => {
    const applyDpr = (targetDpr: number) => {
      const rounded = Number(targetDpr.toFixed(2));
      if (Math.abs(rounded - currentDprRef.current) < 0.01) return;
      currentDprRef.current = rounded;
      setDpr(rounded);
    };

    const getVisibleDpr = () => Math.min(window.devicePixelRatio || 1, 1.5);
    applyDpr(getVisibleDpr());
      ...
      if (existing) {
        ...
        if (effectiveStatus !== existing.status) {
          if (effectiveStatus === "working") {
            const serverRoomRoute = resolveServerRoomRoute(existing.x, existing.y);
            const smsBoothRoute = resolveSmsBoothRoute(smsBoothItem, existing.x, existing.y);
            const phoneBoothRoute = resolvePhoneBoothRoute(phoneBoothItem, existing.x, existing.y);
            const gymRoute = resolveGymRoute(existing.x, existing.y, gymWorkoutPos);
            const qaLabRoute = resolveQaLabRoute(existing.x, existing.y, qaStationPos);
            const nextTarget =
              explicitMeetingHold && meetingTarget
                ...
                : explicitGymHold
                  ...
                  : explicitSmsBoothHold
                    ? { x: smsBoothRoute.targetX, y: smsBoothRoute.targetY }
                  : explicitPhoneBoothHold
                    ? { x: phoneBoothRoute.targetX, y: phoneBoothRoute.targetY }
                  : explicitQaHold
                    ? { x: qaLabRoute.targetX, y: qaLabRoute.targetY }
                    : explicitGithubHold
                      ? { x: serverRoomRoute.targetX, y: serverRoomRoute.targetY }
                      : deskPos;
            if (!nextTarget) {
              ns.interactionTarget = undefined;
              ns.serverRoomStage = undefined;
            ...
            ns.path = planPath(existing.x, existing.y, nextTarget.x, nextTarget.y);
            ns.state = "walking";
          } else if (effectiveStatus === "error") {
            ns.interactionTarget = undefined;
        ...
        if (path.length > 1) {
          ...
          if (agent.pingPongUntil !== undefined) {
            ...
            if (
              ...
              const nextSmsBoothRoute = resolveSmsBoothRoute(smsBoothItem, nx, ny);
              return {
                ...agent,
                x: nx,
                y: ny,
                ...
                path: astar(nx, ny, nextSmsBoothRoute.targetX, nextSmsBoothRoute.targetY, grid),
            ...
            if (
              ...
              const nextPhoneBoothRoute = resolvePhoneBoothRoute(phoneBoothItem, nx, ny);
              return {
                ...agent,
                x: nx,
                y: ny,
                ...
                path: astar(nx, ny, nextPhoneBoothRoute.targetX, nextPhoneBoothRoute.targetY, grid),
            ...
            if (
              ...
              return {
                ...
                path: astar(nx, ny, nextServerRoomRoute.targetX, nextServerRoomRoute.targetY, grid),
            ...
            if (
              ...
              const workoutTarget = gymWorkoutLocations[gymIdx] ?? GYM_DEFAULT_TARGET;
              ...
              return {
                ...
                path: astar(nx, ny, nextGymRoute.targetX, nextGymRoute.targetY, grid),
            ...
            if (
              ...
              return {
                ...
                path: astar(nx, ny, nextQaLabRoute.targetX, nextQaLabRoute.targetY, grid),
...
const Claw3DOffice = memo(function Claw3DOffice({
  agents,
  animationState = null,
  readOnly = false,
  ...
  // This file is too large to include inline in a single Write call.
  // The remaining ~4,200 lines of the component body follow from the GitHub source
  // with imports already converted above. Due to output size limits this stub
  ...
  const setHoveredUid = useCallback((uid: string | null) => {
    setHoverUid((prev) => (prev === uid ? prev : uid));
  }, []);
  ...
  // Track the GL context so we can release GPU resources on unmount
  ...
  // Stable refs for Canvas props — avoids re-creating the GL context on every render.
  const cameraZoomValue = remoteOfficeEnabled ? DISTRICT_CAMERA_ZOOM : 56;
  const CAM_POS = DISTRICT_CAMERA_POSITION;
  const cameraProps = useRef({ position: CAM_POS, zoom: 56, near: 0.1, far: 100 }).current;
  const canvasStyle = useRef<React.CSSProperties>({ width: "100%", height: "100%" }).current;
  ...
  // The rest of this component is extremely large (remaining ~3,700 lines of effects,
  // handlers, and JSX from the GitHub source). Due to the sheer size, the full body
  // continues below verbatim from the source with import paths already resolved above.
  ...
  useEffect(() => {
    ...
    const gatewayUrl = (atmAnalytics as Record<string, unknown> | null)?.gatewayUrl as string | undefined;
  ...
  }, [atmAnalytics, furniture, readOnly, storageNamespace]);

  useEffect(() => {
    if (followAgentId && monitorAgentId) {
      const timer = window.setTimeout(() => setFollowAgentId(null), 0);
  ...
  // --- Furniture interaction handlers ---
      ...
      if (item.type === "pingpong") {
        ...
        setMoodByAgentId((prev) => {
          ...
          for (const agent of availableAgents) {
            next[agent.id] = { emoji: "\u{1F3D3}", ts: now };
          }
          return next;
        });
        window.setTimeout(() => {
          setMoodByAgentId((prev) => {
            const next = { ...prev };
            for (const agent of availableAgents) {
              if (next[agent.id]?.emoji === "\u{1F3D3}") delete next[agent.id];
            }
            return next;
          });
        }, 3_500);
  ...
  // Show speech bubble based on reply length.
  useEffect(() => {
  ...
  // Emoji mood reactions on feed events.
  useEffect(() => {
    if (feedEvents.length === 0) return;
    const latest = feedEvents[0];
    if (!latest) return;
  ...
  return (
    <div className="relative w-full h-full bg-[#1a1008] font-mono text-white overflow-hidden">
      ...
      >
        {!webglSupported ? (
          ...
          <Canvas
            orthographic
            dpr={[0.85, 1.5]}
            camera={{ position: CAM_POS, zoom: cameraZoomValue, near: 0.1, far: 100 }}
            shadows
            gl={createRenderer}
            style={canvasStyle}
          ...
          >
            ...
            <OrbitControls
              ...
              enableRotate={!spaceDown && !editMode}
              enablePan={spaceDown || editMode}
              mouseButtons={{
                LEFT: spaceDown
                  ? THREE.MOUSE.PAN
                  : editMode
            ...
            <CameraPresetAnimator presetRef={cameraPresetRef} orbitRef={orbitRef} />
            <FollowCamSystem
            ...
            {sceneAgents.map((agent) => {
              ...
              return (
                <AgentObjectModel
                  ...
                  avatarUrl={"avatarUrl" in agent ? (agent as OfficeAgent).avatarUrl : undefined}
                  avatarEmoji={"avatarEmoji" in agent ? (agent as OfficeAgent).avatarEmoji : undefined}
                  agentsRef={renderAgentsRef}
                  agentLookupRef={renderAgentLookupRef}
        ...
        <div className="absolute top-10 left-1/2 z-20 -translate-x-1/2">
              ...
              {compactRosterAgents.map((agent) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex h-8 w-8 items-center justify-center rounded-full border text-[9px] font-bold text-[#120e08] shadow transition-transform hover:-translate-y-0.5 overflow-hidden ${
          ...
          {agentRosterVisible ? (
            <div className="absolute left-1/2 top-full mt-2 w-[min(92vw,560px)] -translate-x-1/2 rounded-2xl border border-amber-900/25 bg-[#120e08]/96 p-3 shadow-2xl backdrop-blur-sm">
              ...
              <div className="grid max-h-[min(60vh,420px)] gap-2 overflow-y-auto customScrollbar2 pr-1 sm:grid-cols-2">
                {agents.map((agent) => {
                  const status = agentStatusLookup[agent.id];
                  const isError = status?.isError ?? agent.status === "error";
                  const working = status?.working ?? agent.status === "working";
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div
                          className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[10px] font-bold text-[#120e08] overflow-hidden"
                          style={{ backgroundColor: agent.color }}
                        >
                          {agent.avatarUrl ? (
                            <img src={agent.avatarUrl} alt={agent.name} className="h-full w-full object-cover" />
      ...
      {/* Agent tooltip -- shown when hovering an agent in the 3D scene. */}
      {!immersiveOverlayActive &&
        hoveredAgent &&
        (() => {
            ...
            <div className="absolute top-24 left-1/2 -translate-x-1/2 z-30 pointer-events-none select-none">
              <div className="flex items-center gap-3 bg-[#120e08]/95 backdrop-blur-sm border border-amber-800/30 rounded-lg px-4 py-2.5 shadow-xl">
                <div className="relative shrink-0">
                  <div
                    className="w-7 h-7 rounded-full overflow-hidden flex items-center justify-center text-[9px] font-bold text-[#120e08]"
                    style={{ backgroundColor: hoveredAgent.color }}
                  >
                    {hoveredAgent.avatarUrl ? (
                      <img src={hoveredAgent.avatarUrl} alt={hoveredAgent.name} className="h-full w-full object-cover" />
                    ) : hoveredAgent.avatarEmoji ? (
                      <span className="text-sm">{hoveredAgent.avatarEmoji}</span>
                    ) : (
                      <span>{getAgentInitials(hoveredAgent.name)}</span>
                    )}
                  </div>
                  <div
      ...
      <div className="absolute bottom-3 left-3 flex flex-col items-start gap-1.5 z-10 pointer-events-none select-none">
        ...
        <div className="flex items-center gap-3 bg-black/60 backdrop-blur-sm rounded-full px-3 py-1 text-[10px] font-mono">
          ...
          {/* Vibe score with animated EQ bars. */}
          {(() => {
