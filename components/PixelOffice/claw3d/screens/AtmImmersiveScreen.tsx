...
import type { OfficeUsageAnalyticsParams } from "../externalTypes";

// --- Stubbed analytics view model ---
// TODO: Wire up to real usage analytics when available
...
type CostDailyEntry = { date: string; totalCost: number };
...
type ToolAggregate = { name: string; count: number };
...
type UsageData = {
  ...
  aggregates: {
    ...
    tools: { totalCalls: number; uniqueTools: number; tools: ToolAggregate[] };
    messages: { total: number; user: number; assistant: number; toolResults: number; errors: number };
  };
  loading: boolean;
  error: string | null;
...
function useOfficeUsageAnalyticsViewModel(_params: OfficeUsageAnalyticsParams): {
  usage: UsageData;
  ...
  const usage: UsageData = {
    ...
    aggregates: {
      ...
      messages: { total: 0, user: 0, assistant: 0, toolResults: 0, errors: 0 },
    },
    loading: false,
    error: null,
    lastRefreshedAt: null,
    refresh: async () => {
      // TODO: Implement usage analytics refresh
    },
  };

  return { usage, settingsLoaded: true, startDate, endDate, setStartDate, setEndDate };
}
