...
export class OfficeState {
  ...
  rebuildFromLayout(layout: OfficeLayout, shift?: { col: number; row: number }, skipPathToSeat?: boolean): void {
    ...
    // Do not overwrite character positions — only fix invalid ones so positions are "remembered".
    for (const seat of this.seats.values()) {
      seat.assigned = false
    }

    // First pass: keep existing seat assignments; do not move characters
    for (const ch of this.characters.values()) {
    ...
    // Second pass: assign remaining characters to free seats; do not move them
    for (const ch of this.characters.values()) {
    ...
    // Only fix position when current tile is invalid (out of bounds or not walkable)
    for (const ch of this.characters.values()) {
      const inBounds =
        ch.tileCol >= 0 &&
      ...
      const walkable = inBounds && isWalkable(ch.tileCol, ch.tileRow, this.tileMap, this.blockedTiles)
      if (walkable) continue

      if (ch.seatId) {
        const seat = this.seats.get(ch.seatId)
        if (seat && isWalkable(seat.seatCol, seat.seatRow, this.tileMap, this.blockedTiles)) {
  ...
  setAgentActive(id: number, active: boolean): void {
    ...
    if (ch) {
      ...
      if (!active) {
        ...
        ch.path = []
        ch.moveProgress = 0
        // Snap to a tile so the next render shows them stopped here (avoids "walk then flash back").
        // If mid-walk, snap to the tile nearest current pixel position; otherwise use current tile.
        if (isMidWalk) {
          const col = Math.max(0, Math.min(this.layout.cols - 1, Math.round((ch.x - TILE_SIZE / 2) / TILE_SIZE)))
          const row = Math.max(0, Math.min(this.layout.rows - 1, Math.round((ch.y - TILE_SIZE / 2) / TILE_SIZE)))
          ch.tileCol = col
          ch.tileRow = row
