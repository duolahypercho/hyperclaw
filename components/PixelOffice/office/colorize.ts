 ...
 * 4. Remap lightness into [PATTERN_LIGHTNESS_MIN, PATTERN_LIGHTNESS_MAX] so the pattern always shows
...
export function colorizeSprite(sprite: SpriteData, color: FloorColor): SpriteData {
  ...
  for (const row of sprite) {
    ...
    for (const pixel of row) {
      ...
      // Keep pattern visible: remap into a fixed range so light/dark parts never collapse to one tone
