...
import { ArrowLeft, Save, Loader2, UserPlus, FileClock, LockKeyhole } from "lucide-react";
import {
  getAgent,
  formatUSD,
  formatTokens,
...
import type { AgentState, EnsembleAgent } from "$/components/ensemble";
...
type Tab = "files" | "overview" | "config" | "runs" | "skills" | "mcps" | "cost";
...
const HIRING_DISABLED_TABS = new Set<Tab>(["files", "config", "runs", "skills", "mcps", "cost"]);
...
export default function AgentProfile({ agentId }: { agentId: string }) {
  ...
  const { state } = useAgentStatus(agentId ?? "", { state: rawState as AgentState });
  ...
  return (
    <EnsShell padded={false}>
      ...
      <div className={cn("ens-profile-hero relative overflow-hidden", isHiring && "border-red-400/20 bg-red-500/[0.025]")}>
        ...
        {/* Dirty-state actions stay in the profile hero; agent actions live in the site header. */}
      ...
      <div className="ens-profile-body">
        ...
        >
          ...
          {!isHiring && tab === "mcps" && (
            <AgentMcpsTab agentId={agentId} runtime={agent.runtimeLabel} />
          )}
