...
const RUNTIME_TO_BACKEND_TAB: Partial<Record<RuntimeKind, BackendTab>> = {
...
function runtimeKindToBackendTab(kind: RuntimeKind | undefined): BackendTab {
  return (kind ? RUNTIME_TO_BACKEND_TAB[kind] : undefined) ?? "openclaw";
}
...
export default function EnsembleChat() {
  ...
  const activeAgentRuntime = useMemo(() => {
    const agentId = resolvedActive?.type === "dm" ? resolvedActive.agentId : activeRoomAgentId;
    return runtimeKindToBackendTab(agents.find((agent) => agent.id === agentId)?.kind);
  }, [activeRoomAgentId, agents, resolvedActive]);
  ...
  const widgetStub: Widget = useMemo(() => {
    ...
    return {
      ...
      config: agentId
        ? { agentId, sessionKey, backendTab: activeAgentRuntime, hideTabs: true }
        : { backendTab: activeAgentRuntime, hideTabs: true },
    };
  }, [resolvedActive, activeRoomId, activeRoomAgentId, activeAgentRuntime, fallbackAgentId]);

  const chatSurfaceKey = useMemo(() => {
    if (resolvedActive?.type === "dm") {
      return `dm:${resolvedActive.agentId}:${activeAgentRuntime}`;
    }
    if (resolvedActive?.type === "room" && activeRoomId) {
      return `room:${activeRoomId}:${activeAgentRuntime}`;
    }
    return `fallback:${fallbackAgentId ?? "none"}:${activeAgentRuntime}`;
  }, [activeAgentRuntime, activeRoomId, fallbackAgentId, resolvedActive]);
  ...
  return (
    ...
    >
      <div className="relative shrink-0 w-[24px] h-[24px]">
        <AgentGlyph agent={toSeedShape(agent)} size={24} avatar={agent.avatarData} />
                  ...
                  return (
                    ...
                    >
                      <div className="relative shrink-0 w-[28px] h-[28px]">
                        <AgentGlyph agent={toSeedShape(a)} size={28} avatar={a.avatarData} />
                      </div>
                      <div className="flex-1 min-w-0">
...
function toSeedShape(a: EnsembleAgentView): EnsembleAgent & { real: boolean } {
  return {
    id: a.id,
    name: a.name,
    title: a.title,
