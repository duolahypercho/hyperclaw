...
import {
  getWorkflowTemplate as getStaticWorkflowTemplate,
  isWorkflowTemplateTrigger,
} from "$/lib/workflow-templates";
import {
  bindWorkflowAgentSteps,
  buildTemplateAgentMetadata,
  collectTemplateAgentIds,
  createBridgeStepsFromTemplateSeeds,
  getTemplateAgentAssignments,
  getTemplateAgentRequirements,
  readMetadataStringArray,
  resolveTemplateAgentDefaults,
  templateHasAgentWorkflow,
  type TemplateWithAgentHints,
} from "$/lib/workflow-template-agent-assignment";
import type { WorkflowTemplateAgentRequirement } from "$/lib/workflow-templates";

type Trigger = "manual" | "schedule" | "cron" | "webhook" | "event";

type SeededTemplateAgentSource = {
  id: string;
  suggestedRoles: string[];
  template: TemplateWithAgentHints | null;
};
...
function ProjectEditorInner({
  ...
  // `null` = "Unassigned". Picking a lead from outside the current crew
  // implicitly enrolls them via `assignLead` so the data stays consistent.
  ...
  const [dedicatedAgentOpen, setDedicatedAgentOpen] = useState(false);
  ...
  const [deliveryChannel, setDeliveryChannel] = useState("mission_control");
  const [seededTemplateAgentSource, setSeededTemplateAgentSource] =
    useState<SeededTemplateAgentSource | null>(null);
  const [templateAgentDefaultsApplied, setTemplateAgentDefaultsApplied] =
    useState(false);
  const [templateMissingAgentIds, setTemplateMissingAgentIds] = useState<string[]>([]);
  const [optimisticAgentIds, setOptimisticAgentIds] = useState<string[]>([]);
  ...
  useEffect(() => {
    setSeededTemplateAgentSource(null);
    setTemplateAgentDefaultsApplied(false);
    setTemplateMissingAgentIds([]);
    setOptimisticAgentIds([]);
    setAgentAssignments({});
    setCreatingRequirementId(null);
    setDeliveryChannel("mission_control");
  }, [projectId, templateId]);

  useEffect(() => {
    if (projectId) return;
    if (!templateId || !seededTemplateAgentSource || templateAgentDefaultsApplied) return;

    const rememberedAgentIds = collectTemplateAgentIds(seededTemplateAgentSource.template);
    if (rememberedAgentIds.length === 0) {
      setTemplateMissingAgentIds([]);
      setTemplateAgentDefaultsApplied(true);
      return;
    }

    const defaults = resolveTemplateAgentDefaults(
      seededTemplateAgentSource.template,
      availableAgents.map((agent) => agent.id),
    );
    setTemplateMissingAgentIds(defaults.missingAgentIds);

    // If the roster is still hydrating, wait before deciding remembered ids are
    // unavailable. The next agent sync will re-run this effect.
    if (availableAgents.length === 0 && defaults.crew.length === 0) return;

    if (defaults.crew.length > 0) {
      setCrew((prev) => (
        prev.length > 0
          ? prev
          : Array.from(new Set(defaults.crew))
      ));
    }
    if (defaults.leadAgentId) {
      setLeadAgentId((prev) => prev ?? defaults.leadAgentId);
    }
    setTemplateAgentDefaultsApplied(true);
  }, [
    availableAgents,
    projectId,
    seededTemplateAgentSource,
    templateAgentDefaultsApplied,
    templateId,
  ]);
  ...
  useEffect(() => {
    ...
    if (staticSeed) {
      ...
      if (staticSeed.defaults?.cadence) setCadence(staticSeed.defaults.cadence);
      if (staticSeed.defaults?.time) setTime(staticSeed.defaults.time);
      if (staticSeed.defaults?.cron) setCron(staticSeed.defaults.cron);
      setDeliveryChannel(staticSeed.defaults?.deliveryChannel ?? "mission_control");
      setSeededTemplateAgentSource({
        id: staticSeed.id,
        suggestedRoles: staticSeed.suggestedRoles ?? [],
        template: staticSeed as TemplateWithAgentHints,
      });
      return;
    }
    void getPersistedWorkflowTemplate(templateId)
      .then((seed) => {
        ...
        setTrigger(isWorkflowTemplateTrigger(seed.preview?.trigger) ? seed.preview.trigger : "manual");
        if (typeof seed.metadata?.cadence === "string") setCadence(seed.metadata.cadence);
        if (typeof seed.metadata?.time === "string") setTime(seed.metadata.time);
        if (typeof seed.metadata?.cron === "string") setCron(seed.metadata.cron);
        if (typeof seed.metadata?.deliveryChannel === "string") {
          setDeliveryChannel(seed.metadata.deliveryChannel);
        }
        setAgentAssignments(getTemplateAgentAssignments(seed));
        setSeededTemplateAgentSource({
          id: seed.id,
          suggestedRoles: readMetadataStringArray(seed.metadata, "suggestedRoles"),
          template: seed,
        });
      })
      .catch(() => {
  ...
  const templateSuggestedRoles = seededTemplateAgentSource?.suggestedRoles ?? [];
  const templateAgentRequirements = useMemo(
    () => getTemplateAgentRequirements(seededTemplateAgentSource?.template),
    [seededTemplateAgentSource],
  );
  const creatingRequirement = useMemo<WorkflowTemplateAgentRequirement | null>(
    () => (
      creatingRequirementId
        ? templateAgentRequirements.find((requirement) => requirement.id === creatingRequirementId) ?? null
        : null
    ),
    [creatingRequirementId, templateAgentRequirements],
  );
  const seededTemplateHasAgentWorkflow = seededTemplateAgentSource
    ? templateHasAgentWorkflow(
        seededTemplateAgentSource.template,
        templateSuggestedRoles,
      )
    : false;
  const templateAssignmentActive = Boolean(
    templateId && !projectId && seededTemplateHasAgentWorkflow,
  );
  const workflowUsesCrew = Boolean(
    projectId || !templateId || seededTemplateHasAgentWorkflow || crew.length > 0 || templateAgentRequirements.length > 0,
  );
  ...
  const assignLead = (nextId: string | null) => {
    setLeadAgentId(nextId);
    if (nextId && !crew.includes(nextId)) {
      setCrew((prev) => (prev.includes(nextId) ? prev : [...prev, nextId]));
    }
  };

  const assignRequirementAgent = (requirementId: string, agentId: string | null) => {
    setAgentAssignments((prev) => {
      const next = { ...prev };
      if (agentId) next[requirementId] = agentId;
      else delete next[requirementId];
      return next;
    });
    if (agentId) {
      setCrew((prev) => (prev.includes(agentId) ? prev : [...prev, agentId]));
      setLeadAgentId((prev) => prev ?? agentId);
    }
  };

  const openRequirementAgentDialog = (requirementId: string) => {
    setCreatingRequirementId(requirementId);
    setDedicatedAgentOpen(true);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const workflowUsesAgents = workflowUsesCrew;
      const safeCrew = workflowUsesAgents
        ? crew.filter((agentId) => validAgentIds.has(agentId))
        : [];
      const safeAgentAssignments = workflowUsesAgents
        ? Object.fromEntries(
            Object.entries(agentAssignments).filter(([, agentId]) => safeCrew.includes(agentId)),
          )
        : {};
      ...
      // `assignLead` already auto-enrolls the lead into the crew, but we
      // double-check here as a safety net in case the lead was lifted from
      // a stale prop and the crew subsequently shrank.
      const safeLeadId =
        workflowUsesAgents && leadAgentId && safeCrew.includes(leadAgentId)
          ? leadAgentId
          : null;
      const agentBinding = {
        crew: safeCrew,
        leadAgentId: safeLeadId,
        suggestedRoles: templateSuggestedRoles,
        agentAssignments: safeAgentAssignments,
        agentRequirements: templateAgentRequirements,
        deliveryChannel,
      };
      ...
      if (existing) {
        ...
        const toAdd = safeCrew.filter((id) => !existingAgentIds.includes(id));
        const toRemove = existingAgentIds.filter((id) => !safeCrew.includes(id));
        ...
        if (project) {
          ...
          const memberResults = await Promise.all(safeCrew.map((agentId) => addMember(project.id, agentId)));
          if (memberResults.some((ok) => !ok)) {
          ...
          if (templateId && templateSeedSource === "sqlite") {
            const cloned = await cloneWorkflowTemplate(templateId, project.id, name);
            if (!cloned?.id) {
              throw new Error("Failed to clone workflow template");
            }
            if (seededTemplateHasAgentWorkflow) {
              await updateWorkflowTemplate(cloned.id, {
                metadata: buildTemplateAgentMetadata(cloned.metadata, agentBinding),
                steps: cloned.steps
                  ? bindWorkflowAgentSteps(cloned.steps, agentBinding)
                  : undefined,
              });
            }
          } else {
            const staticTemplateSeed =
              templateSeedSource === "static" && templateId
                ? getStaticWorkflowTemplate(templateId)
                : null;
            const baseMetadata = {
              reads: safeReads,
              writes: safeWrites,
            ...
            const triggerStep = {
              ...
              stepType: "manual_trigger" as const,
              dependsOn: [],
              position: 0,
            ...
            const agentStep = {
              ...
              stepType: "agent_task" as const,
              dependsOn: [`${project.id}-trigger`],
              position: 1,
              metadata: { reads: safeReads, writes: safeWrites },
            };
            const outputStep = {
              id: `${project.id}-output`,
              name: "Deliver result",
              stepType: "notification" as const,
              dependsOn: workflowUsesAgents
                ? [`${project.id}-agent-step`]
                : [`${project.id}-trigger`],
              position: workflowUsesAgents ? 2 : 1,
              metadata: { channel: deliveryChannel },
            };
            const staticTemplateSteps = staticTemplateSeed?.steps?.length
              ? createBridgeStepsFromTemplateSeeds(
                  staticTemplateSeed,
                  safeAgentAssignments,
                  { deliveryChannel },
                )
              : [];
            const steps = staticTemplateSteps.length > 0
              ? bindWorkflowAgentSteps(staticTemplateSteps, agentBinding)
              : workflowUsesAgents
                ? bindWorkflowAgentSteps([triggerStep, agentStep, outputStep], agentBinding)
                : [triggerStep, outputStep];
            const template = await createWorkflowTemplate({
              ...
              metadata: workflowUsesAgents
                ? buildTemplateAgentMetadata(baseMetadata, agentBinding)
                : baseMetadata,
  ...
  return (
        ...
        <div className="space-y-5">
          ...
          <Card>
            <CardHeader className="pb-3 flex flex-row items-center justify-between space-y-0">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={() => setDedicatedAgentOpen(true)}
                >
                  <UserPlus size={12} />
            ...
            <CardContent className="pb-0">
              {templateAssignmentActive && (
                <div className="mb-4 rounded-xl border border-primary/30 bg-primary/5 p-3">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      <div className="text-[11px] font-medium uppercase tracking-wider text-primary">
                        Template agent requirements
                      </div>
                      <p className="mt-1 text-xs leading-5 text-muted-foreground">
                        Assign each required agent slot. Use an existing agent,
                        or create a new one from the template&apos;s embedded soul,
                        role, skills, and runtime instructions.
                      </p>
                    </div>
                    {templateSuggestedRoles.length > 0 && (
                      <div className="flex max-w-full flex-wrap gap-1.5 sm:max-w-[260px] sm:justify-end">
                        {templateSuggestedRoles.map((role) => (
                  ...
                  {templateMissingAgentIds.length > 0 && (
                    <p className="mt-2 text-[11px] leading-5 text-amber-500">
                      This template remembered unavailable agents:{" "}
                      <span className="font-mono">
                        {templateMissingAgentIds.join(", ")}
                      </span>
                      . Choose replacements before saving.
                    </p>
                  )}
                  {templateAgentRequirements.length > 0 && (
                    <div className="mt-3 grid gap-3">
                      {templateAgentRequirements.map((requirement) => {
                        const assignedAgentId = agentAssignments[requirement.id] ?? "__none__";
                        return (
                          <div
                            key={requirement.id}
                          ...
                          >
                            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                              <div className="min-w-0 space-y-1">
                                <div className="flex flex-wrap items-center gap-2">
                                  <span className="text-sm font-medium">
                                    {requirement.emoji} {requirement.label}
                                  </span>
                                  <Badge variant="outline" className="text-[10px]">
                                    {requirement.role}
                                  </Badge>
                                  <Badge variant="secondary" className="text-[10px]">
                                    {requirement.agentBlueprint.runtime}
                                  </Badge>
                                </div>
                              ...
                              <div className="flex w-full flex-col gap-2 md:w-[280px]">
                                ...
                                >
                                  <SelectTrigger aria-label={`Attach existing agent for ${requirement.label}`}>
                                    <SelectValue
                                      placeholder={
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  className="gap-1.5"
                                  onClick={() => openRequirementAgentDialog(requirement.id)}
                                >
                                  <UserPlus size={12} />
                                  Create {requirement.defaultName}
                                </Button>
                              </div>
                            </div>
                            <p className="mt-2 text-[11px] leading-5 text-muted-foreground">
                              Step prompt: {requirement.prompt}
                            </p>
                          </div>
                        );
                      })}
              ...
              <div className="mb-4 flex flex-col gap-2 rounded-lg border border-primary/30 bg-primary/5 p-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    ...
                    workflow card and Mission Control header. Picking a lead
                    from outside the crew adds them automatically.
                  </p>
                </div>
                <Select
                  value={leadAgentId ?? "__none__"}
                  onValueChange={(value) =>
                    assignLead(value === "__none__" ? null : value)
                  }
                  disabled={availableAgents.length === 0}
                >
                  <SelectTrigger
                  ...
                  <SelectContent>
                    <SelectItem value="__none__">Unassigned</SelectItem>
                    {availableAgents.map((agent) => {
                      const inCrew = crew.includes(agent.id);
                      return (
                        <SelectItem key={agent.id} value={agent.id}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  <Label
                    htmlFor="workflow-delivery-channel"
                    className="text-sm font-medium"
                  >
                    Output channel
                  </Label>
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Mission Control is the safe default. External channels can
                    be wired later through OpenClaw channel targets.
                  </p>
                </div>
                <Select value={deliveryChannel} onValueChange={setDeliveryChannel}>
                  <SelectTrigger
                    id="workflow-delivery-channel"
                    className="w-full sm:w-[220px]"
                    aria-label="Select workflow output channel"
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="mission_control">Mission Control</SelectItem>
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground mb-3">
                {workflowUsesCrew
                  ...
                  : "Intelligence data collections this workflow may read or write during a run."}
              </p>
              <form
                className="mb-3 flex gap-2"
                onSubmit={(event) => {
        ...
        <aside>
          <Card className="sticky top-4">
            ...
            <CardContent className="space-y-1">
              ...
              {workflowUsesCrew && <SumRow label="Crew" value={`${crew.length} agents`} />}
    ...
    <AddAgentDialog
      open={dedicatedAgentOpen}
      onOpenChange={(open) => {
        setDedicatedAgentOpen(open);
        if (!open) setCreatingRequirementId(null);
      }}
      existingAgents={availableAgents}
      initialRuntime={creatingRequirement?.agentBlueprint.runtime}
      initialName={creatingRequirement?.agentBlueprint.defaultName}
      initialRole={creatingRequirement?.agentBlueprint.role}
      initialDescription={creatingRequirement?.agentBlueprint.description}
      initialEmoji={creatingRequirement?.agentBlueprint.emoji}
      initialSoulContent={creatingRequirement?.agentBlueprint.soulContent}
      initialWorkspaceInstructions={creatingRequirement?.agentBlueprint.workspaceInstructions}
      initialUserNotes={creatingRequirement?.agentBlueprint.userNotes}
      initialSkills={creatingRequirement?.agentBlueprint.skills}
      initialSoulTemplateSlug={creatingRequirement?.agentBlueprint.soulTemplateSlug}
      initialAgentBlueprint={creatingRequirement?.agentBlueprint}
      onSuccess={(agentId) => {
        setOptimisticAgentIds((prev) => (prev.includes(agentId) ? prev : [...prev, agentId]));
        setTemplateMissingAgentIds((prev) => prev.filter((id) => id !== agentId));
        setCrew((prev) => (prev.includes(agentId) ? prev : [...prev, agentId]));
        setLeadAgentId((prev) => prev ?? agentId);
        if (creatingRequirementId) {
          assignRequirementAgent(creatingRequirementId, agentId);
        }
      }}
    />
    </>
