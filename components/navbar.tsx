...
interface NavItemDef {
  ...
  glyphAgent?: Pick<EnsembleAgent, "id" | "kind" | "emoji" | "name">;
  agentStatus?: string;
  isDisabled?: boolean;
}
  ...
  return (
    ...
    >
      <span className="relative shrink-0 inline-flex w-[20px] h-[20px]">
        <AgentGlyph agent={item.glyphAgent} size={20} />
  ...
  const navSections: NavSectionDef[] = useMemo(() => {
    const toolNavItems = visibleTools.map((tool) => ({
      ...
      isActive:
        ...
        (tool.id === "team" && activeTool?.id === "agent"),
    }));
                ...
                <NavTooltip label={item.name} collapsed={true}>
                  <Button
                    ...
                    className={cn(
                      ...
                      (activeTool?.id === item.id || (item.id === "team" && activeTool?.id === "agent")) &&
