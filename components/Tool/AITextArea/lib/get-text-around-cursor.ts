...
import { EditorAutocompleteState } from "./editor-autocomplete-state";
...
export interface EditorDescendantState {
  selection: BaseRange;
...
export function getFullEditorTextWithNewlines(editor: Editor): string {
  const fullDocumentRange: Range = {
    anchor: Editor.start(editor, [] as Path),
    focus: Editor.end(editor, [] as Path),
  };
...
export function getDescendantAroundSelection(
  editor: Editor
): EditorDescendantState | null {
  const { selection } = editor;
  if (!selection) {
    return null;
  }
  ...
  // Create ranges for before, selected, and after content
  const beforeRange: Range = {
    anchor: Editor.start(editor, []),
    focus: wellOrderedSelection.anchor,
  };
  ...
  // Get descendants for each range
  const beforeNodes = Array.from(
    Editor.nodes(editor, {
      at: beforeRange,
      match: (n) =>
        Element.isElement(n) &&
        (n.type === "paragraph" || n.type === "list_item"),
    })
  ).map(([node]) => node as Descendant);

  // Only get selectedNodes if there's actually a selection
  const selectedNodes = isCollapsed
    ? []
    : Array.from(
        Editor.nodes(editor, {
          at: wellOrderedSelection,
          match: (n) =>
            Element.isElement(n) &&
            (n.type === "paragraph" || n.type === "list_item"),
        })
  ...
  return {
    selection: wellOrderedSelection,
    descendantsBeforeCursor: beforeNodes,
    selectedDescendants: selectedNodes,
    descendantsAfterCursor: afterNodes,
  };
}
