...
async function updateLocalTaskStatus(id: string, status: KanbanStatus) {
  const data = await bridgeInvoke("get-todo-data") as { tasks?: Array<Record<string, unknown>> } | null;
  const tasks = Array.isArray(data?.tasks) ? data.tasks : [];
  const now = new Date().toISOString();
  await bridgeInvoke("save-todo-data", {
    todoData: {
      ...data,
      tasks: tasks.map((task) => {
        ...
        return {
          ...
          status,
          updatedAt: now,
          finishedAt: status === "completed" ? task.finishedAt ?? now : null,
        };
      }),
    },
  });
...
export function TaskDetailPanel({ task, onStatusChange }: { task: TaskDetailPanelTask; onStatusChange?: (taskId: string, newStatus: KanbanStatus) => void | Promise<any> }) {
  ...
  const handleChangeStatus = useCallback(async (newStatus: KanbanStatus) => {
    ...
    try {
      if (onStatusChange) {
        // Delegate to parent when a live issue board owns the state.
        await onStatusChange(task._id, newStatus);
      } else {
        await updateLocalTaskStatus(task._id, newStatus);
      }
    } catch (e) {
