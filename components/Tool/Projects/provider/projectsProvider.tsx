...
export function ProjectsProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await bridgeInvoke("project-list", {});
  ...
  const createProject = useCallback(async (name: string, description: string, emoji: string) => {
    ...
    try {
      setError(null);
      const res = await bridgeInvoke("project-create", { name, description, emoji });
      const project = normalizeProject(unwrapBridgeData(res, "project"));
      if (project) {
        replaceProjectSnapshot(optimisticProject.id, project);
        return project;
      }
