...
Local credentials are intended for `localhost` desktop/dev use. If you expose
the dashboard on a LAN or public host, leave
`HYPERCLAW_ALLOW_NETWORK_LOGIN=false` and use a proper OAuth provider or add
your own local password gate before sharing access.
