...
import {
  UseCopanionChatOptions,
  useCopanionChat as useCopanionChatInternal,
  UseCopanionChatReturn as UseCopanionChatReturnInternal,
} from "./use-copanion-chat_internal";

// Create a type that excludes message-related properties from the internal type
export type UseCopanionChatReturn = Omit<
  UseCopanionChatReturnInternal,
  | "messages"
  | "sendMessage"
  | "suggestions"
  | "setSuggestions"
 ...
 */
export function useCopanionChat(options: UseCopanionChatOptions = {}): UseCopanionChatReturn {
  const {
    visibleMessages,
    appendMessage,
    reloadMessages,
    ...
    loadConversation,
  } = useCopanionChatInternal(options);
