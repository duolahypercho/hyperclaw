...
import { Message, parseJson, HyperchoAgentDiscoveryError } from "@OS/AI/shared";
import { useAsyncCallback } from "@OS/AI/core/components/error-boundary/error-utils";
import { useToast } from "@/components/ui/use-toast";
import { useCopanionRuntimeClient } from "./use-copanion-runtime-client";
import { useMessagesTap } from "@OS/AI/core/copanion-messages";
import { Message as GqlMessage } from "@OS/AI/runtime-client";
...
export function useCoAgent<T = any>(
  ...
  useEffect(() => {
    if (
      ...
      const agentError = new HyperchoAgentDiscoveryError({
        agentName: name,
  ...
  const { sendMessage, runChatCompletion } = useCopanionChat();
  const headers = {
  ...
  const runtimeClient = useCopanionRuntimeClient({
    ...
    publicApiKey: copanionApiConfig.publicApiKey,
    headers,
  ...
  const setState = useCallback(
    (newState: T | ((prevState: T | undefined) => T)) => {
      ...
      let coagentState: CoagentState = getCoagentState({
        coagentStates,
        name,
        options,
      });
  ...
  useEffect(() => {
    ...
    setCoagentStatesWithRef((prev) => {
      const existing = prev[name] ?? {
        name,
        state: isInternalStateManagementWithInitial(options)
  ...
  return useMemo(() => {
    ...
    return {
      ...
      setState: isExternalStateManagement(options)
        ? options.setState
        : setState,
...
export function startAgent(name: string, context: CopanionContextParams) {
...
export function stopAgent(name: string, context: CopanionContextParams) {
...
const getCoagentState = <T>({
  ...
  if (coagentStates[name]) {
    ...
    return {
      name,
      state: isInternalStateManagementWithInitial<T>(options)
        ? options.initialState
        : {},
