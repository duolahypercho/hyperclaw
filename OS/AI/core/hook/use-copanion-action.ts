/**
 * Example usage of usecopanionAction with complex parameters:
 *
 * @example
 * usecopanionAction({
 *   name: "myAction",
 ...
 *   example: "Handle a meeting by booking or canceling",
 *   parameters: [
 *     {
 *       name: "meeting",
 *       type: "string",
...
/**
 * <img src="https://cdn.copanionkit.ai/docs/copanionkit/images/use-copanion-action/usecopanionAction.gif" width="500" />
 * `usecopanionAction` is a React hook that you can use in your application to provide
 * custom actions that can be called by the AI. Essentially, it allows the copanion to
 * execute these actions contextually during a chat, based on the user's interactions
 * and needs.
 *
 * Here's how it works:
 *
 * Use `usecopanionAction` to set up actions that the copanion can call. To provide
 * more context to the copanion, you can provide it with a `description` (for example to explain
 ...
 * copanionKit takes care of automatically inferring the parameter types, so you get type safety
 ...
 * ## Generative UI
 *
 * This hooks enables you to dynamically generate UI elements and render them in the copanion chat. For more information, check out the [Generative UI](/guides/generative-ui) page.
 */
import { createElement, Fragment, useEffect, useRef } from "react";
import { Parameter, randomId } from "@OS/AI/shared";
...
// We implement usecopanionAction dependency handling so that
...
export function useCopanionAction<const T extends Parameter[] | [] = []>(
  ...
  // const { currentlyActivatingHitlActionMessageIdRef } = usecopanionContext() as any; // <-- REMOVE THIS FOR NOW
  ...
  if (
    ...
    action.render = ((
      props: ActionRenderProps<T> & { messageId?: string }
    ): React.ReactElement => {
      ...
      const waitProps = {
        ...
        ? ActionRenderPropsNoArgsWait<T>
        : ActionRenderPropsWait<T>;
      ...
      const isNoArgsRenderWait = (
        ...
        props: ActionRenderPropsNoArgsWait<T>
      ) => React.ReactElement => {
    ...
    if (actions[idRef.current]) {
      ...
      if (typeof action.render === "function") {
        if (chatComponentsCache.current !== null) {
          ...
          chatComponentsCache.current.actions[action.name] =
            action.render as any;
        }
      }
    }
  }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // This is because we currently don't have access to the messages array in copanionContext.
    ...
    isFrontendAction(action) ? action.example : undefined,
    action.name,
