...
} from "@OS/AI/shared/utils/errors";
...
export class RemoteLangGraphEventSource {
  ...
  private getCurrentContent(event: LangGraphEvent) {
    // Type guard to check if event has data property
    if (!("data" in event)) {
      return null;
    }
  ...
  processLangGraphEvents() {
    ...
    return this.eventStream$.pipe(
      ...
      mergeMap((acc): RuntimeEvent[] => {
        ...
        if (
          acc.event &&
          acc.event.event == LangGraphEventTypes.OnChatModelStream
        ) {
          if ("metadata" in acc.event && acc.event.metadata) {
            const metadata = acc.event.metadata as Record<string, any>;
            if ("copilotkit:emit-tool-calls" in metadata) {
              shouldEmitToolCalls = metadata["copilotkit:emit-tool-calls"];
            }
            if ("copilotkit:emit-messages" in metadata) {
              shouldEmitMessages = metadata["copilotkit:emit-messages"];
            }
          }
        }

        if (acc.event && acc.event.event === LangGraphEventTypes.OnInterrupt) {
          events.push({
        ...
        if (
          acc.event &&
          acc.event.event === LangGraphEventTypes.OnCopilotKitInterrupt
        ) {
          events.push({
            type: RuntimeEventTypes.MetaEvent,
        ...
        if (
          acc.event &&
          acc.event.event === LangGraphEventTypes.OnCopilotKitError
        ) {
        ...
        const responseMetadata = acc.event
          ? this.getResponseMetadata(acc.event)
          : null;

        // Tool call ended: emit ActionExecutionEnd
        if (
          acc.isToolCallEnd &&
          acc.lastToolCallName &&
          acc.lastToolCallId &&
          ...
          case LangGraphEventTypes.OnChatModelStream:
            if (
              ...
              events.push({
                ...
                parentMessageId: acc.lastMessageId ?? undefined,
              });
            }
