...
const createMarkdownComponents = (isUser?: boolean): Components => {
  ...
  const components: Components = {
        ...
        return (
          <code
            className={`text-sm ${isUser
                ...
                : "bg-secondary border border-solid border-border text-foreground"
        ...
        >
          <div
            className={`flex items-center justify-between px-3 py-1.5 border-b ${
              isUser
                ? "border-border/10 bg-secondary-foreground/5"
          ...
          <div className="overflow-auto p-3 customScrollbar2">
            <pre
              className={`text-sm ${isUser ? "text-secondary-foreground" : "text-foreground"
                } whitespace-pre-wrap`}
            >
    ...
    h1: ({ children, ...props }) => (
      <h1
        className={`text-xl font-medium my-4 select-text ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    h2: ({ children, ...props }) => (
      <h2
        className={`text-lg font-medium my-2 select-text ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    h3: ({ children, ...props }) => (
      <h3
        className={`text-md font-medium my-2 select-text ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    h4: ({ children, ...props }) => (
      <h4
        className={`text-sm font-medium my-2 select-text ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    h5: ({ children, ...props }) => (
      <h5
        className={`text-xs font-medium my-2 select-text ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    p: ({ children, ...props }) => (
      <p
        className={`my-2 first:mt-0 text-sm last:mb-0 select-text break-words ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    blockquote: ({ children, ...props }) => (
      <blockquote
        className={`border-l-4 ${isUser ? "border-secondary-foreground/30" : "border-border/30"
          } pl-4 my-2 italic ${isUser ? "text-secondary-foreground/80" : "text-muted-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    ul: ({ children, ...props }) => (
      <ul
        className={`list-disc list-outside pl-6 my-2 space-y-1 text-sm leading-relaxed break-words [&_ul]:ml-0 [&_ul]:mt-1 [&_ul]:list-[circle] [&_ul]:pl-5 [&_ol]:ml-0 [&_ol]:mt-1 [&_ol]:pl-5 ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    ol: ({ children, ...props }) => (
      <ol
        className={`list-decimal list-outside pl-6 my-2 space-y-1 text-sm leading-relaxed break-words [&_ul]:ml-0 [&_ul]:mt-1 [&_ul]:pl-5 [&_ol]:ml-0 [&_ol]:mt-1 [&_ol]:pl-5 ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    li: ({ children, ...props }) => (
      <li
        className={`my-1 pl-0.5 break-words [&>p]:inline [&>ul]:mt-1 [&>ol]:mt-1 [&>p:not(:first-child)]:block [&>p:not(:first-child)]:pl-5 [&>p:not(:first-child)]:relative [&>p:not(:first-child)]:before:content-['•'] [&>p:not(:first-child)]:before:absolute [&>p:not(:first-child)]:before:left-0 ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    strong: ({ children, ...props }) => (
      <strong
        className={`font-semibold text-sm break-words overflow-wrap-anywhere ${isUser ? "text-secondary-foreground" : "text-foreground"
          }`}
        {...props}
      >
        {children}
    ...
    hr: ({ ...props }) => (
      <hr
        className={`my-4 h-[1px] ${isUser ? "bg-secondary-foreground/30" : "bg-border"
          }`}
        {...props}
      />
    ),
    ...
    thead: ({ children, ...props }) => (
      <thead
        className={isUser ? "bg-secondary-foreground/10" : "bg-muted/50"}
        {...props}
      >
        {children}
      </thead>
    ...
    th: ({ children, ...props }) => (
      <th
        className={`px-3 py-2 text-left text-xs font-semibold border border-border first:border-l-0 last:border-r-0 ${
          isUser ? "text-secondary-foreground/80" : "text-foreground/80"
        }`}
        {...props}
      >
        {children}
    ...
    td: ({ children, ...props }) => (
      <td
        className={`px-3 py-2 text-sm border border-border border-solid first:border-l-0 last:border-r-0 ${
          isUser ? "text-secondary-foreground/90" : "text-foreground/90"
        }`}
        {...props}
      >
        {children}
